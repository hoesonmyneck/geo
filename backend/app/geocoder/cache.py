...
# Путь к кэшу: сначала пробуем volume /app/data/cache, иначе рядом с кодом
_volume_path = Path("/app/data/cache/geocoder.sqlite")
_fallback_path = Path(__file__).parent.parent / "data" / "cache.sqlite"
DB_PATH = _volume_path if _volume_path.parent.exists() else _fallback_path
